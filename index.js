...
const tabs = ["All", "Korea", "China", "Japan"]
...
for (const tab of tabs) {
  ...
  btn.addEventListener("click" , () => {  
      const items = []
      if (tab !== 'All') {
          items.push(...(menu.filter((i) => i.category === tab) || []))
      } else {
          items.push(...(menu || []))
      }
      addItems(items)
...
function addItems (items) {
    menuDom.replaceChildren([])
    items.forEach(items => {
       let createMenu = document.createElement("div");
        createMenu.classList.add("menu-items", "col-lg-6", "col-sm-12");
       ...
       menuDom.appendChild(createMenu)
    })    
}
